...
def executePartC(socket, link, current_range):
    ...
    end_time = time.time()
    print("Total time in seconds for range: {}".format(current_range))
    print(end_time - start_time)

    writeToFile('big{}.txt'.format(current_range), message)
    return None
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    for range in [10, 100, 1000, 10000, 15000]:
        executePartC(s, hiddenLink, range)
